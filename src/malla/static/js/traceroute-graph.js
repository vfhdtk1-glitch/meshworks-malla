/* Traceroute graph page logic extracted from template */
(function(window, document) {
'use strict';
    const helpers = window.PageHelpers || {};
    let TracerouteGraph = window.TracerouteGraph || {};
    if (!window.TracerouteGraph) {
        TracerouteGraph = {
            refresh: () => {},
            refreshGraph: () => {},
            centerGraph: () => {},
        ...
        window.TracerouteGraph = TracerouteGraph;
    }
    const d3 = window.d3;
    if (!d3) {
        console.warn('TracerouteGraph: d3 library is not available; skipping graph initialisation.');
        return;
    }
    ...
    document.addEventListener('DOMContentLoaded', function() {
        // Ensure sidebar toggle works even without jQuery
        function ensureSidebarToggleBinding() {
            try {
                const toggleBtn = document.getElementById('toggleSidebar');
                const sidebar = document.getElementById('sidebar');
                if (helpers && helpers.ensureVanillaSidebarToggle) {
                    helpers.ensureVanillaSidebarToggle(toggleBtn, sidebar);
                    return;
                }
                if (toggleBtn && sidebar && !toggleBtn.__vanillaBound) {
                    const vanillaHandler = () => {
                        sidebar.classList.toggle('collapsed');
                        const icon = toggleBtn.querySelector('i');
                        const isMobile = window.innerWidth <= 768;
                        if (icon) {
                            icon.className = sidebar.classList.contains('collapsed')
                                ? (isMobile ? 'bi bi-chevron-up' : 'bi bi-chevron-left')
                                : (isMobile ? 'bi bi-chevron-down' : 'bi bi-chevron-right');
                        }
                    };
                    toggleBtn.addEventListener('click', vanillaHandler);
                    toggleBtn.__vanillaHandler = vanillaHandler;
                    toggleBtn.__vanillaBound = true;
                }
            } catch (_) { /* ignore */ }
        }
    
        // Wait for jQuery to be available, with a safe fallback
        function waitForJQuery() {
            ensureSidebarToggleBinding();
            if (typeof $ !== 'undefined') {
                initializeInterface();
                loadGraph();
            } else {
                // Fallback: minimal init without jQuery
                setTimeout(() => {
                    ensureSidebarToggleBinding();
                    try { loadGraph(); } catch (_) {}
                }, 100);
...
function initializeInterface() {
    ...
    if (helpers && helpers.removeVanillaSidebarToggle) {
        helpers.removeVanillaSidebarToggle(toggleButtonEl);
        ...
        delete toggleButtonEl.__vanillaHandler;
        toggleButtonEl.__vanillaBound = false;
    }
        ...
        setTimeout(() => {
            ...
            if (helpers && helpers.updateSidebarIcon) {
                const sidebar = document.getElementById('sidebar');
                helpers.updateSidebarIcon(toggleButtonEl, sidebar);
            }
        }, 300);
    });
...
function loadGraph() {
    ...
    const params = (helpers && helpers.createFormParams)
        ? helpers.createFormParams(formEl)
        : (() => {
            ...
            if (formEl) {
                formEl.querySelectorAll('input, select, textarea').forEach((element) => {
                    ...
                    if ((type === 'checkbox' || type === 'radio')) {
                        if (element.checked) {
                            const value = element.value && element.value !== 'on' ? element.value : 'true';
                            fallback.append(element.name, value);
                        }
                        return;
                    }
                    if (element.value !== '' && element.value !== null) {
                        fallback.append(element.name, element.value);
                    }
                });
            }
            return fallback;
    ...
    function renderGraph(data) {
        try {
            ...
            if (!data.nodes || data.nodes.length === 0) {
                console.log('No nodes found in data — creating placeholder SVG');
                // Still create an SVG so tests awaiting #networkGraph svg pass
                const fallbackWidth = Math.max(800, window.innerWidth - 400);
    ...
    function displaySearchResults(results) {
        ...
        container.innerHTML = results.map((node, index) => {
            const nodeIdHex = node.id.toString(16).padStart(8, '0');
        return `
            <div class="search-result-item" onclick="TracerouteGraph.selectSearchResult(${index})">
...
async function loadPrimaryChannels() {
    if (helpers && helpers.loadPrimaryChannels) {
        return helpers.loadPrimaryChannels('primary_channel');
    }
    try {
    ...
    TracerouteGraph = tracerouteApi;
    window.TracerouteGraph = tracerouteApi;
})(window, document);
